...
This makes the cache the primary leakage surface:
  1. Temporal leak:     U queries D before revocation and it is cached.
                        U after revocation or A asks again and gets cache hit.
...
class SemanticCache:
    ...
    def get(
        ...
        NO access control check is performed here. That is the vulnerability.
        """
        now = time.time()
        best_sim: float = -1.0
        ...
        for entry in self._entries:
            # Skip expired entries but do not delete them because they still occupy
            # memory and could be reactivated if TTL logic is later changed.
