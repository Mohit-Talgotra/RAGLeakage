...
from src.rbac import AccessControlStore
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if row6 and row7:
        w = 36
        hdr = f"  {'Signal':<22} {'Step 6 — D exists (restricted)':<{w}} {'Step 7 — topic absent':<{w}} Distinguishable?"
        print(f"\n{hdr}")
        print("  " + "-" * (len(hdr) - 2))

        def row(label, v6, v7, thresh):
            diff = str(v6) != str(v7)
            badge = "YES ✓" if diff else "no"
            if thresh is not None:
                try:
                    diff = abs(float(v6) - float(v7)) > thresh
                    badge = f"YES ✓ (Δ={abs(float(v6)-float(v7)):.4f})" if diff else "marginal"
                except (TypeError, ValueError):
                    pass
            print(f"  {label:<22} {str(v6):<{w}} {str(v7):<{w}} {badge}")

        row("Cache hit",       row6.cache_hit,        row7.cache_hit,        None)
        row("Similarity score", row6.similarity_score, row7.similarity_score, 0.05)
    ...
    content_leak_attacker = row6 is not None and row6.cache_hit and row6.ground_truth_restricted
    sidechannel_leaked = (
    ...
    print("  RUN SUMMARY")
    print("=" * 68)
    print(f"\n  Temporal leakage (cache, post-revocation):     {'✓ CONFIRMED' if temporal_cache_leaked else '✗ not observed'}")
    print(f"  Content leakage  (attacker received D):        {'✓ CONFIRMED' if content_leak_attacker  else '✗ not observed'}")
    print(f"  Side-channel leakage (distinguishable signal): {'✓ CONFIRMED' if sidechannel_leaked     else '✗ not observed'}")
